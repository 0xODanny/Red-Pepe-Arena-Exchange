...
# ----------------------------------- BUY ----------------------------------- #
    def buy(self, token_address, token_to_spend):
        ...
        max_priority_fee_per_gas = self.web3.to_wei(2, 'gwei') # Priority fee to include the transaction in the block
        max_fee_per_gas = int((1.4* base_fee_per_gas)) + max_priority_fee_per_gas

        print(base_fee_per_gas)
        print(max_priority_fee_per_gas)
        print(max_fee_per_gas)

        txn = self.contract.functions.swapAVAXForExactTokens(
            #x[0]/10**BASE_DECIMALS,
            x[1],
            [
        ...
        if txn_receipt.status == 1:
            return x[1] * 0.99, True, tx
        else:
            return x[1] * 0.99, False, tx
